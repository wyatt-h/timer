...
add column if not exists speaker_default_seconds integer
check (speaker_default_seconds is null or speaker_default_seconds between 1 and 86400);

alter table public.event_runtime
add column if not exists panel_status public.timer_status,
add column if not exists panel_remaining_seconds numeric
check (panel_remaining_seconds is null or panel_remaining_seconds >= 0),
add column if not exists panel_ends_at timestamptz;
