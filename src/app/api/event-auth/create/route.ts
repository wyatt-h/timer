 ...
 * event cannot exist.
 *
 * The recovery code is generated here, returned exactly once in this response, and
 * stored only as a hash — nothing can show it again afterwards.
 */
...
export async function POST(request: Request) {
  try {
    ...
    const [passwordHash, recoveryCodeHash] = await Promise.all([
      hashSecret(parsed.data.password),
    ...
    return apiSuccess(
      { ...result.payload, recoveryCode: formatRecoveryCode(recoveryCode) },
      [session.cookie],
      201,
    );
  } catch (error) {
